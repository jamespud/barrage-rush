...
public class AuthServiceImpl implements AuthService {
  ...
  public AuthResponse register(RegisterRequest registerRequest) {
    // 验证验证码
    boolean captchaValid = verifyCaptcha(registerRequest.getCaptchaId(),
    ...
    // 验证邮箱是否已存在
    if (registerRequest.getEmail() != null && userRepository.existsByEmail(
  ...
  public AuthResponse refreshToken(RefreshTokenRequest refreshTokenRequest) {
    ...
    boolean isSensitiveOperation = refreshTokenRequest.isSensitiveOperation();
    ...
    try {
      ...
      // 验证刷新令牌
      // 如果是敏感操作，则需要验证token版本号
      boolean isValid;
      if (isSensitiveOperation) {
        isValid = jwtUtil.validateTokenAndVersion(refreshToken, userDetails);
        if (!isValid) {
          log.warn("用户 [{}] 尝试使用过期版本的token进行敏感操作", username);
          throw new BarrageException("您的密码已更改，请重新登录");
        }
      } else {
        isValid = jwtUtil.validateToken(refreshToken, userDetails);
        if (!isValid) {
  ...
  public boolean changePassword(String username, String oldPassword, String newPassword) {
    ...
    if (!passwordEncoder.matches(oldPassword, user.getPassword())) {
      throw new BarrageException("原密码不正确");
    }

    // 更新密码
    user.setPassword(passwordEncoder.encode(newPassword));

    // 更新token版本号，使所有已颁发的refresh token失效
